...
export default function ComponentsLayout({ children }: { children: React.ReactNode }) {
  const { connectionDetails: connectionDetailsProvider } = useConnectionDetails();

  const pathname = usePathname();
  const room = React.useMemo(() => new Room(), []);

  React.useEffect(() => {
    if (room.state === 'disconnected' && connectionDetailsProvider) {
      Promise.all([
        room.localParticipant.setMicrophoneEnabled(true, undefined, {
          preConnectBuffer: true,
        }),
        connectionDetailsProvider.generate().then(connectionDetails => (
  ...
  }, [room, connectionDetailsProvider]);
